...
// TypeDescriptor
TypeDescriptor::TypeDescriptor(TypeDescriptorKind typeKind, DATA_TYPE type): kind(typeKind) {
  if(kind == SCALAR_TYPE_DESCRIPTOR) {
    properties.dataType = type;
  } else if(kind == ARRAY_TYPE_DESCRIPTOR){
    properties.arrayProperties.elementType = type;
    properties.arrayProperties.init();
  } else {
    fprintf(stderr, "TypeDescriptorKind Error");
    exit(1);
  }
}

DATA_TYPE TypeDescriptor::getDataType() const{
  if(kind == SCALAR_TYPE_DESCRIPTOR)
    return properties.dataType;
  else
    return properties.arrayProperties.elementType;
}

void TypeDescriptor::addDimension(int sizeInDimension){
  assert(getKind() == ARRAY_TYPE_DESCRIPTOR);

  properties.arrayProperties.sizeInEachDimension[properties.arrayProperties.dimension] = sizeInDimension;
  properties.arrayProperties.dimension++;
}

int TypeDescriptor::getDimension() const{
  assert(getKind() == ARRAY_TYPE_DESCRIPTOR);

  return properties.arrayProperties.dimension;
}

int TypeDescriptor::getArrayDimensionSize(int idx) const{
  assert(kind == ARRAY_TYPE_DESCRIPTOR);
  assert(idx < getDimension() && idx >= 0);

  return properties.arrayProperties.sizeInEachDimension[idx];
}

bool TypeDescriptor::isCompatible(const TypeDescriptor &b) const{
  if(kind != b.kind) return false;
  switch(kind){
    case ARRAY_TYPE_DESCRIPTOR:
      if(properties.arrayProperties.isCompatible(b.properties.arrayProperties, true)) 
        return true;
      else
        return false;
    case SCALAR_TYPE_DESCRIPTOR:
      if(getDataType() == FLOAT_TYPE && b.getDataType() == INT_TYPE)
        return true;
      else if(getDataType() == INT_TYPE && b.getDataType() == FLOAT_TYPE)
        return true;
      else
        return properties.dataType == b.properties.dataType;
    default:
      fprintf(stderr, "Unknown Type Descriptor: %d\n", kind);
      return false;
  }
}

TypeDescriptorKind TypeDescriptor::getKind() const{
  return kind;
}

void TypeDescriptor::getSubArrayProperty(int dimension, TypeDescriptor **temp) const{
  assert(getKind() == ARRAY_TYPE_DESCRIPTOR);
  if(getDimension() == dimension)
    *temp = new TypeDescriptor(SCALAR_TYPE_DESCRIPTOR, getDataType());
  else {
    *temp = new TypeDescriptor(ARRAY_TYPE_DESCRIPTOR, getDataType());
    for(int i=dimension; i<getDimension(); i++)
      (*temp)->addDimension(getArrayDimensionSize(i));
  }
}
